...
LSH 协议是一种面向虚拟世界的视图同步协议，定义了场景数据的标准化表示和同步机制。该协议定义了一套标准的事件类型和数据结构，用于在多视图（结构视图、2D 视图、3D 视图、列表视图）之间保持数据一致性。
...
# category 默认配置
CATEGORY_DEFAULTS = {
    "room": {"can_contain": True, "has_bounds": True, "defines_coords": True},
    "device": {"is_interactive": True},
    "furniture": {"can_contain": True, "is_interactive": True},
    "item": {},
    "vehicle": {"can_contain": True, "is_interactive": True, "has_bounds": True, "defines_coords": True},
}
...
| category | 说明 | 默认属性 |
|----------|------|----------|
| `room` | 房间 | can_contain, has_bounds, defines_coords |
| `floor` | 楼层 | can_contain, has_bounds, defines_coords |
| `device` | 设备 | is_interactive |
| `furniture` | 家具 | can_contain, is_interactive |
| `item` | 物品 | 无 |
| `vehicle` | 载具 | can_contain, is_interactive, has_bounds, defines_coords |
| `...` | 用户自定义 | 自定义 |
...
- [ ] **代码实现**：更新 SceneElement 模型代码，实现属性查询机制
- [ ] **迁移指南**：完善从 v2.x 到 v3.0 的迁移文档
